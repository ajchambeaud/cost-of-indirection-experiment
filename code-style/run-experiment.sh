...
if [ "$VARIANT" = "all" ]; then
  VARIANTS=(variant-a-inline variant-b-abstracted)
else
  VARIANTS=("$VARIANT")
fi
...
# Get the root of the git repo (parent of code-style/)
GIT_ROOT=$(cd .. && pwd)
...
for V in "${VARIANTS[@]}"; do
  ...
  for RUN in $(seq 1 "$RUNS"); do
    ...
    (cd "$GIT_ROOT" && git checkout -- "code-style/$V/")
    # Clean any new files the agent might have created
    (cd "$GIT_ROOT" && git clean -fd "code-style/$V/" 2>/dev/null || true)
    ...
    GIT_FILES_CHANGED=$(cd "$GIT_ROOT" && git diff --name-only "code-style/$V/" | wc -l | tr -d ' ')
    GIT_LINES_ADDED=$(cd "$GIT_ROOT" && git diff --numstat "code-style/$V/" | awk '{s+=$1} END {print s+0}')
    GIT_NEW_FILES=$(cd "$GIT_ROOT" && git ls-files --others --exclude-standard "code-style/$V/" | wc -l | tr -d ' ')

    # Run baseline tests
    BASELINE_PASS=false
    if ./run-tests.sh "$V" 2>&1 | tail -1 | grep -q "PASS"; then
      BASELINE_PASS=true
    fi

    # Run task tests
    TASK_PASS=false
    if ./run-tests.sh "$V" "$TASK" 2>&1 | tail -1 | grep -q "PASS"; then
      TASK_PASS=true
    fi
